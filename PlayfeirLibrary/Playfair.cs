...
namespace PlayfairLibrary
{
    public class Playfair
    {
        ...
        {
            char[,] keySquare = new char[6, 6];
            string defaultKeySquare = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ 12";
            ...
            tempKey = tempKey.Substring(0, 36);

            for (int i = 0; i < 36; ++i)
                keySquare[(i / 6), (i % 6)] = tempKey[i];
        ...
        {
            return new char[] { keySquare[row, Mod((col1 + encipher), 6)], keySquare[row, Mod((col2 + encipher), 6)] };
        }

        private  char[] SameColumn(ref char[,] keySquare, int col, int row1, int row2, int encipher)
        {
            return new char[] { keySquare[Mod((row1 + encipher), 6), col], keySquare[Mod((row2 + encipher), 6), col] };
        }

        private  char[] SameRowColumn(ref char[,] keySquare, int row, int col, int encipher)
        {
            return new char[] { keySquare[Mod((row + encipher), 6), Mod((col + encipher), 6)], keySquare[Mod((row + encipher), 6), Mod((col + encipher), 6)] };
        }
